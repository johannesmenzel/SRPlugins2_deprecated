...
def  main():
  ...
  installer = "\installer\SRChangesTest Installer.exe"
   
  if demo:
    installer = "\installer\SRChangesTest Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRChangesTest manual.pdf" 
  ]
  ...
  ZIPNAME = "SRChangesTest-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRChangesTest-v" + FULLVERSIONSTR + "-win-demo.zip"
