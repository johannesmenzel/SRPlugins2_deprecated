...
class SRChannelAppViewController: UIViewController {
  @IBOutlet weak var playButton: UIButton!
  @IBOutlet weak var auContainerView: UIView!
  
  var playEngine: SimplePlayEngine!
  var mAudioUnitViewController: SRChannelViewController!
  ...
  override func viewDidLoad() {
    ...
    AUAudioUnit.registerSubclass(IPlugAUAudioUnit.self, as: desc, name:"iPlug: Local SRChannel", version: UInt32.max)
  ...
  func embedPlugInView() {
    ...
    let pluginURL = builtInPlugInsURL.appendingPathComponent("SRChannelAppExtension.appex")
    let appExtensionBundle = Bundle(url: pluginURL)
    
    let storyboard = UIStoryboard(name: "SRChannel-iOS-MainInterface", bundle: appExtensionBundle)
    mAudioUnitViewController = storyboard.instantiateInitialViewController() as? SRChannelViewController
