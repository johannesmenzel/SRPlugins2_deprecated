...
class SRCompressorAppViewController: UIViewController {
  @IBOutlet weak var playButton: UIButton!
  @IBOutlet weak var auContainerView: UIView!
  
  var playEngine: SimplePlayEngine!
  var mAudioUnitViewController: SRCompressorViewController!
  ...
  override func viewDidLoad() {
    ...
    AUAudioUnit.registerSubclass(IPlugAUAudioUnit.self, as: desc, name:"iPlug: Local SRCompressor", version: UInt32.max)
  ...
  func embedPlugInView() {
    ...
    let pluginURL = builtInPlugInsURL.appendingPathComponent("SRCompressorAppExtension.appex")
    let appExtensionBundle = Bundle(url: pluginURL)
    
    let storyboard = UIStoryboard(name: "SRCompressor-iOS-MainInterface", bundle: appExtensionBundle)
    mAudioUnitViewController = storyboard.instantiateInitialViewController() as? SRCompressorViewController
