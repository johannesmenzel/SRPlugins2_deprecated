...
def  main():
  ...
  installer = "\installer\SRCompressor Installer.exe"
   
  if demo:
    installer = "\installer\SRCompressor Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRCompressor manual.pdf" 
  ]
  ...
  ZIPNAME = "SRCompressor-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRCompressor-v" + FULLVERSIONSTR + "-win-demo.zip"
