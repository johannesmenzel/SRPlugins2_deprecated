...
def  main():
  ...
  installer = "\installer\SRManConvo Installer.exe"
   
  if demo:
    installer = "\installer\SRManConvo Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRManConvo manual.pdf" 
  ]
  ...
  ZIPNAME = "SRManConvo-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRManConvo-v" + FULLVERSIONSTR + "-win-demo.zip"
