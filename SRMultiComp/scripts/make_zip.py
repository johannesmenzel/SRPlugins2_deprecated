...
def  main():
  ...
  installer = "\installer\SRMultiComp Installer.exe"
   
  if demo:
    installer = "\installer\SRMultiComp Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRMultiComp manual.pdf" 
  ]
  ...
  ZIPNAME = "SRMultiComp-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRMultiComp-v" + FULLVERSIONSTR + "-win-demo.zip"
