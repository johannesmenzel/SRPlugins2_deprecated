...
class SROverdriveAppViewController: UIViewController {
  @IBOutlet weak var playButton: UIButton!
  @IBOutlet weak var auContainerView: UIView!
  
  var playEngine: SimplePlayEngine!
  var mAudioUnitViewController: SROverdriveViewController!
  ...
  override func viewDidLoad() {
    ...
    AUAudioUnit.registerSubclass(IPlugAUAudioUnit.self, as: desc, name:"iPlug: Local SROverdrive", version: UInt32.max)
  ...
  func embedPlugInView() {
    ...
    let pluginURL = builtInPlugInsURL.appendingPathComponent("SROverdriveAppExtension.appex")
    let appExtensionBundle = Bundle(url: pluginURL)
    
    let storyboard = UIStoryboard(name: "SROverdrive-iOS-MainInterface", bundle: appExtensionBundle)
    mAudioUnitViewController = storyboard.instantiateInitialViewController() as? SROverdriveViewController
