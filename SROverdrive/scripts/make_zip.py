...
def  main():
  ...
  installer = "\installer\SROverdrive Installer.exe"
   
  if demo:
    installer = "\installer\SROverdrive Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SROverdrive manual.pdf" 
  ]
  ...
  ZIPNAME = "SROverdrive-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SROverdrive-v" + FULLVERSIONSTR + "-win-demo.zip"
