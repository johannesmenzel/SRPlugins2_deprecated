...
class SRReverbAppViewController: UIViewController {
  @IBOutlet weak var playButton: UIButton!
  @IBOutlet weak var auContainerView: UIView!
  
  var playEngine: SimplePlayEngine!
  var mAudioUnitViewController: SRReverbViewController!
  ...
  override func viewDidLoad() {
    ...
    AUAudioUnit.registerSubclass(IPlugAUAudioUnit.self, as: desc, name:"iPlug: Local SRReverb", version: UInt32.max)
  ...
  func embedPlugInView() {
    ...
    let pluginURL = builtInPlugInsURL.appendingPathComponent("SRReverbAppExtension.appex")
    let appExtensionBundle = Bundle(url: pluginURL)
    
    let storyboard = UIStoryboard(name: "SRReverb-iOS-MainInterface", bundle: appExtensionBundle)
    mAudioUnitViewController = storyboard.instantiateInitialViewController() as? SRReverbViewController
