...
def  main():
  ...
  installer = "\installer\SRReverb Installer.exe"
   
  if demo:
    installer = "\installer\SRReverb Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRReverb manual.pdf" 
  ]
  ...
  ZIPNAME = "SRReverb-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRReverb-v" + FULLVERSIONSTR + "-win-demo.zip"
