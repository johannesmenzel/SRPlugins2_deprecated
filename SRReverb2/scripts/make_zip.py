...
def  main():
  ...
  installer = "\installer\SRReverb2 Installer.exe"
   
  if demo:
    installer = "\installer\SRReverb2 Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRReverb2 manual.pdf" 
  ]
  ...
  ZIPNAME = "SRReverb2-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRReverb2-v" + FULLVERSIONSTR + "-win-demo.zip"
