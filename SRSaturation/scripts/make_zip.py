...
def  main():
  ...
  installer = "\installer\SRSaturation Installer.exe"
   
  if demo:
    installer = "\installer\SRSaturation Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRSaturation manual.pdf" 
  ]
  ...
  ZIPNAME = "SRSaturation-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRSaturation-v" + FULLVERSIONSTR + "-win-demo.zip"
