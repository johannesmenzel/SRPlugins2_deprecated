...
if %1 == 1 (echo Making SRSaturation Windows DEMO VERSION distribution ...) else (echo Making SRSaturation Windows FULL VERSION distribution ...)
...
REM - msbuild SRSaturation-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild SRSaturation.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild SRSaturation.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\SRSaturation.aaxplugin\Contents\Win32\SRSaturation.aaxplugin --out .\build-win\aax\bin\SRSaturation.aaxplugin\Contents\Win32\SRSaturation.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\SRSaturation.aaxplugin\Contents\x64\SRSaturation.aaxplugin --out .\build-win\aax\bin\SRSaturation.aaxplugin\Contents\x64\SRSaturation.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\SRSaturation.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\SRSaturation.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "SRSaturation Installer" ".\installer\SRSaturation Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\SRSaturation Installer.exe" ".\installer\SRSaturation Demo Installer.exe"
REM -del ".\installer\SRSaturation Installer.exe"
REM -)
