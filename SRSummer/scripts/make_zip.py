...
def  main():
  ...
  installer = "\installer\SRSummer Installer.exe"
   
  if demo:
    installer = "\installer\SRSummer Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRSummer manual.pdf" 
  ]
  ...
  ZIPNAME = "SRSummer-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRSummer-v" + FULLVERSIONSTR + "-win-demo.zip"
