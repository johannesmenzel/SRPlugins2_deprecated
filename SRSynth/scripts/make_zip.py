...
def  main():
  ...
  installer = "\installer\SRSynth Installer.exe"
   
  if demo:
    installer = "\installer\SRSynth Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRSynth manual.pdf" 
  ]
  ...
  ZIPNAME = "SRSynth-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRSynth-v" + FULLVERSIONSTR + "-win-demo.zip"
