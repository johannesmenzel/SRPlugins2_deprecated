...
def  main():
  ...
  installer = "\installer\SRTestBed Installer.exe"
   
  if demo:
    installer = "\installer\SRTestBed Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRTestBed manual.pdf" 
  ]
  ...
  ZIPNAME = "SRTestBed-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRTestBed-v" + FULLVERSIONSTR + "-win-demo.zip"
